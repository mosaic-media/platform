...
func RunOutboxAtomicityContract(t *testing.T, newDeps Factory) {
	...
	// The two subtests below re-prove the same commit-together / rollback-both
	// guarantee, but resolve the stores through slice 13's uniform
	// contracts.Store[T] mechanism instead of the named tx accessors — the path
	// MAD-001 moves command handlers onto. They are additive: the accessor-based
	// subtests above stay, so both paths are proven to carry atomicity
	// identically. Verification here is through the contract read path, the only
	// adapter-agnostic option (the suite imports contracts and domain only); the
	// stricter raw-SQL proof that bypasses the stores lives in the PostgreSQL
	// module's own outbox_atomicity_test.go, which also drives its writes
	// through Store[T].
	t.Run("Store[T] commit persists state and event together", func(t *testing.T) {
		d := newDeps(t)
		c := ctx(t)
		err := d.UnitOfWork.WithinTx(c, func(c context.Context, tx contracts.Tx) error {
			...
			if _, err := users.Create(c, newUser("u-store-1", "ivy", now)); err != nil {
				return err
			}
			return outbox.Append(c, domain.OutboxEvent{Event: domain.Event{
				ID: "e-store-1", Type: "user.created", Payload: []byte("ivy"), OccurredAt: now,
			}})
		})
		if err != nil {
			t.Fatalf("WithinTx: %v", err)
		}
		if _, err := d.Users.FindByID(c, "u-store-1"); err != nil {
			t.Fatalf("user resolved via Store[T] should be committed: %v", err)
		}
		events, err := d.Outbox.ListUnpublished(c, 10)
		if err != nil {
			t.Fatalf("ListUnpublished: %v", err)
		}
		if len(events) != 1 || events[0].ID != "e-store-1" {
			t.Fatalf("expected one committed event via Store[T], got %+v", events)
		}
	})

	t.Run("Store[T] rollback persists neither state nor event", func(t *testing.T) {
		d := newDeps(t)
		c := ctx(t)
		sentinel := errors.New("forced rollback via Store[T] path")
		err := d.UnitOfWork.WithinTx(c, func(c context.Context, tx contracts.Tx) error {
			users, err := contracts.Store[contracts.UserStore](tx)
			if err != nil {
				return err
			...
			if _, err := users.Create(c, newUser("u-store-2", "judy", now)); err != nil {
				return err
			}
			if err := outbox.Append(c, domain.OutboxEvent{Event: domain.Event{
				ID: "e-store-2", Type: "user.created", Payload: []byte("judy"), OccurredAt: now,
			}}); err != nil {
				return err
			}
			return sentinel // force rollback after both Store[T]-resolved writes
		})
		if !errors.Is(err, sentinel) {
			t.Fatalf("WithinTx error = %v, want sentinel", err)
		}
		if _, err := d.Users.FindByID(c, "u-store-2"); contracts.CategoryOf(err) != contracts.NotFound {
			t.Fatalf("user must not be committed after Store[T] rollback, got err=%v", err)
		}
		events, err := d.Outbox.ListUnpublished(c, 10)
		if err != nil {
			t.Fatalf("ListUnpublished: %v", err)
		...
		if len(events) != 0 {
			t.Fatalf("event must not be committed after Store[T] rollback, got %+v", events)
		}
	})
