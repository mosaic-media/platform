...
// Serialized across a process boundary it becomes something a module holds — a
// string in another process's memory, which that process can keep, write to
// disk, or use later.
//
// platform#39 rejected a signed short-TTL bearer token: a TTL is a window in
// which a retained value still works, and retention is the entire risk. A
// five-minute token means five minutes during which a module that kept it can
// act as the user who invoked it, for work that user never asked for. A handle
// that stops resolving the instant the invocation returns has no window at all,
// nothing to tune and no clock-skew story.
//
...
// handle grants no more than the session behind it; it only says which session,
// and only for as long as the invocation lasts.
...
//
// The revoke is returned rather than exposed as a method taking a handle, so the
// only way to obtain one is to have minted it — a call site cannot revoke
...
//
// Unguessability is not the primary control — the table is, and a handle that is
// not in it fails whatever it looks like. It matters because the alternative is a
// counter, which is both guessable and replayable across restarts: a stale handle
// from a previous boot would occasionally resolve to somebody else's invocation.
//
