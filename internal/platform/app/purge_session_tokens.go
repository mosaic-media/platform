...
//
// Its own action rather than a reuse of user.session.revoke, because deleting a
// token that expired last week is not revoking a session, and conflating them
// would put a maintenance sweep behind a permission about ending somebody's
// access. It sits in the superuser preset beside the other install-level
// actions; in practice the only caller is the system principal.
...
//
// The access-token table is the fastest-growing thing in the schema: one row per
// client per ten minutes, forever, and nothing else removes them. The rows are
// useless the moment they expire — validation checks the expiry — so this is
// disk and index size rather than correctness.
//
// It is a DELETE rather than a partition drop, unlike telemetry retention: the
// tables are small relative to telemetry and the rows are removed by expiry
// rather than by day.
