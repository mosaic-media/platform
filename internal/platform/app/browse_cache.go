...
//
// Calling an aggregator live takes seconds when it works — measured at 9.6s cold
// and 2.6s warm for one search — so a slow or absent source means a slow or
// wrong screen. Keeping only what succeeded also renders a full library as
// "Nothing to show yet", presenting a transient failure as a configuration
// mistake.
//
// These reads serve the last good answer and revalidate around it. They are
// stale-while-revalidate, not optimistic UI: optimistic rendering shows a
// predicted write outcome before the server confirms it, and nothing here
// predicts anything.
...
// immediately, it only triggers a background refresh — so it trades how quickly
// a new title appears against how often a quiet home screen re-renders itself
// under a viewer. A revalidation costs a full provider round trip plus a pushed
// region, so re-running one per navigation would spend the cost this exists to
// avoid.
//
// A constant rather than configuration: a config field is a promise to keep a
// reload class correct forever.
...
//
// The catalog list is snapshotted as well as its contents, and it has to be: an
// addon's catalogs come out of a manifest fetched over the network, so a cold or
// unreachable addon has no catalogs at all — and a home screen with no catalogs
// renders "add an addon in Settings" to an install that has one.
func (s *Service) BrowseCatalogs(ctx context.Context, q BrowseCatalogsQuery) (BrowseCatalogsResult, error) {
	...
	// general metadata sources yields two rows of the same films under two names.
	// The floor is consulted only when the preferred sources produced nothing at
	// all — and with a snapshot in front of them, a source that is merely down
	// produces its last answer rather than nothing.
...
// serial gather is a sum of latencies rather than a max. Per provider rather
// than through fanOut because this needs each source's provenance as well as its
// rows, and a gather that concatenates cannot say which source was the stale
// one.
func (s *Service) browseCatalogTier(ctx context.Context, q BrowseCatalogsQuery, entries []CatalogProviderEntry) (BrowseCatalogsResult, error) {
	...
	for i, e := range entries {
		...
		go func() {
			...
			// skipped, and a settings read that fails means storage is broken
			// and must fail the query rather than quietly emptying a screen.
...
// and says so when it fails. Do not add paging or filter fields here.
...
func (s *Service) BrowseCatalogItems(ctx context.Context, q BrowseCatalogItemsQuery) (BrowseCatalogItemsResult, error) {
	...
	// In-library is re-derived, never restored. It is a fact about this install's
	// own graph rather than about the source's answer, so a snapshot taken before
	// a title was added would otherwise draw it as absent. The read is local, so
...
// return different things and the policy is identical. It stays in this package
// rather than becoming a general utility: the policy is specific to browsing a
// source, not to caching.
func cacheFirst[T any](
	...
	items, err := read(ctx)
	if err != nil {
		// The failure must be counted and named, never discarded. Dropping it
		// makes a downed source indistinguishable from an empty library, and only
		// one of those is the user's to act on.
...
// viewer cannot see which row came from where.
