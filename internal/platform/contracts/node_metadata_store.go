...
type NodeMetadataStore interface {
	...
	// **NotFound**, which a caller reads as "render what the node itself
	// carries" rather than as an error — the distinction matters because an
	// empty document and an absent one mean different things about whether the
	// provider was ever asked.
