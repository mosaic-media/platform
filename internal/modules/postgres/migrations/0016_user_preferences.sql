...
--
-- Per-user settings a user chooses for themselves: expert mode (platform#36) is
...
--     versioned and carrying reload classes (platform#7). A user's own choices
...
--     whole install (platform#17). These are per person.
...
CREATE TABLE IF NOT EXISTS user_preferences (
    ...
    -- platform#9 has it for attribute payloads: the Platform stores it, and the
...
-- and nothing that should block the removal — unlike content, where platform#9
