...
type ContractSet struct {
	...
	// installed extension modules (platform#51), read at boot to re-adopt them.
	...
	// class (platform#28). Direct rather than transaction-scoped: it is written
	...
	// (platform#26) — a detail screen's resume offset and the continue-watching
	// rail. Writes go through the UnitOfWork.
	PlaybackStates contracts.PlaybackStateStore
	// TelemetryQueries reads stored telemetry back (platform#36).
	...
	// Jobs is the background-work queue (platform#13). Pool-backed rather than
	...
	// (platform#62) — what a library detail renders from.
