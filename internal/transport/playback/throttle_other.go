...
//
// The consequence: on a non-unix host the transcode is not throttled, so a
// playback's segment directory grows to the whole release instead of a window.
// Eviction behind the playhead still runs, so it is bounded by how far ahead the
// encoder gets rather than unbounded — but it is not the window the unix build
// has.
const (
