...
// PosterCard, Section, SettingsFrame … — expressed as a tree of primitives. The
// Platform pushes it to a client on connect (below), so the design system is
// server-owned: a client ships only the primitive vocabulary and the generic
// expander, and renders whatever definitions the Platform sends. Redesigning a
// component is a Platform change, not a client release.
//
// **It comes from the contract, which is where components are authored.** It
// used to be a JSON file in this repository, regenerated by dumping the React
// runtime's own bundled copy — so the components lived in a client, the
// published contract carried a second set of the same names, and the two drifted
// (three of the four that existed in both had). There is one library now and it
// is `mosaic-sdui`'s; this reads it rather than keeping a copy.
var definitionsLibrary = mustDefinitions()
...
// client is handed (ADR 0040's UI-library tier, the half that was never built).
// Values used to live in the client's own stylesheet, so a re-skin was a client
// release and the contract's copy drifted into fiction.
var tokenSet = mustTokens()

// tokensEvent names the Event that carries it. A client applies the payload over
// its bootstrap skin before rendering, so what it draws with is what the server
// said, not what it shipped with.
...
func tokensMsg() *sessionv1.ServerMessage {
	return &sessionv1.ServerMessage{Body: &sessionv1.ServerMessage_Event{Event: &sessionv1.Event{Type: tokensEvent, Payload: tokenSet}}}
}
