...
// Package graphql is the GraphQL command and query projection transport. It
// is a transport and projection surface only: every resolver in this package
// calls an application command or query service (internal/platform/app) and
// nothing else. Resolvers must never open a database connection or import
// internal/modules/postgres directly; boundary_test.go statically enforces
// this.
//
// Auth, Users, Permissions and Configuration are real, backed by application
// services. Jobs and Health are schema stubs — no Jobs or cross-component
// diagnostics application service exists yet, so their resolvers return a
// flagged "not implemented" error rather than inventing one (see jobs.go,
// health.go). Diagnostics (support bundle) and product/media queries are out
// of scope: the former awaits the diagnostics model, the latter is deferred
