...
func run() error {
	...
	root := newRootLogger(resource, fileSink)
	...
	moduleRegistry := registerBuiltinModules(boot)
	...
	set, err := connectAndMigrate(dsn, migrations)
	if err != nil {
		return err
	}
	defer set.Pool.Close()

	if err := reportStorageHealth(set, boot); err != nil {
		return err
	}
	...
	if err := drainOutboxAtBoot(worker, boot); err != nil {
		return err
	}

	diagRegistry := newDiagnosticsRegistry(set, bus, worker)
	...
	ensureTelemetryPartitions(telemetryStore, boot)
	...
	selection, err := moduleSelection(moduleClient)
	if err != nil {
		return err
	}

	capRegistry, err := composeCapabilities(selection, moduleClient, boot)
	if err != nil {
		return err
	}

	if err := reportExtensionRepositoryTrust(boot); err != nil {
		return err
	}

	reportEgressContainment(boot)
	...
	installer, err := newExtensionInstaller()
	if err != nil {
		return err
	}
	instanceIdentity := newInstanceIdentity()
	...
	svc := app.NewService(app.Deps{
		...
		// The resolution register: what is operationally wrong with this install
		// now, as durable typed statements, each held until resolved and
		// carrying the Suggestions a client renders as remedies (platform#74).
		Issues:   set.Issues,
	...
	applyConfigWaitingForRestart(svc, boot)

	// Close the construction cycle: a spawned module registers into the same
	// registry a compiled-in one does, and nothing above the registry can tell
	// the difference.
	...
	if err := adoptInstalledExtensions(extManager); err != nil {
		return err
	}
	...
	artworkSigner, err := newArtworkSigner()
	if err != nil {
		return err
	}

	playbackSealer, err := newPlaybackSealer()
	if err != nil {
		return err
	}

	playbackRemuxer, playbackProber := newPlaybackTooling(boot)
	...
	if err := reconcileOwnerRole(set, boot); err != nil {
		return err
	}

	if err := bootstrapAdmin(set, boot); err != nil {
		return err
	}
	...
	jobRuntime := startBackgroundJobs(serveCtx, svc, set, resource.BootID, boot)
	...
	flushQueryableSinks(bufferedSink, spanSink, boot)

	boot.Info("exiting cleanly")
	return serveErr
}

// newRootLogger comes first because everything after it has something worth
// saying and the only alternative available would be fmt.Printf (platform#31). Two
// sinks, neither optional (platform#36): the console keeps the boot narration
// legible to a human at a terminal, and the file is the durable record that
// survives a crash and still works when the database does not.
func newRootLogger(resource telemetry.Resource, fileSink telemetry.Sink) *telemetry.Logger {
...
// registerBuiltinModules registers the built-in modules the same way an external
// Module would be discovered. Postgres is the first, required storage module.
func registerBuiltinModules(boot *telemetry.Logger) *builtin.Registry {
...
// connectAndMigrate is the bootstrap phase, bounded. Migrate fails fast on a
// missing, incompatible or partially applied schema, so this is the startup gate
// that refuses to run against a mismatched database. Bracketing it with
// migrations.Begin/Complete is what makes the migration status endpoint reflect
// real state — Running while this is in flight, Complete or Failed once it
// returns — rather than a value invented for the endpoint alone.
func connectAndMigrate(dsn string, migrations *runtime.MigrationTracker) (*postgres.ContractSet, error) {
	bootCtx, bootCancel := context.WithTimeout(context.Background(), 60*time.Second)
	pool, err := postgres.Connect(bootCtx, dsn)
	if err != nil {
		bootCancel()
		return nil, fmt.Errorf("storage connect failed: %w", err)
	}
	...
	if migrateErr != nil {
		pool.Close()
		return nil, fmt.Errorf("storage migration failed: %w", migrateErr)
	}

	return postgres.New().Bind(pool), nil
}

// reportStorageHealth records what the storage module says about itself. Detail
// is free text a reporter attached, so it is classified rather than printed: it
// is exactly where a DSN with a password in it would surface.
func reportStorageHealth(set *postgres.ContractSet, boot *telemetry.Logger) error {
...
// drainOutboxAtBoot publishes what the last run committed but did not deliver.
func drainOutboxAtBoot(worker *events.Worker, boot *telemetry.Logger) error {
...
// newDiagnosticsRegistry aggregates real component health per the diagnostics
// model, backing both the local structured log and the /readyz endpoint.
func newDiagnosticsRegistry(set *postgres.ContractSet, bus *events.Bus, worker *events.Worker) *diagnostics.Registry {
...
// ensureTelemetryPartitions creates the partitions the queryable sink writes
// into, once at boot, before the jobs runner exists — because the very first
// records this process writes are the ones describing its own start-up. From here
// on the same work is a scheduled job (platform#36) and this call is not repeated.
//
// A failure is not fatal. A Platform that refuses to start because it cannot
// store its own logs has turned an observability problem into an outage, and the
// file sink still has everything.
func ensureTelemetryPartitions(telemetryStore *postgres.TelemetryStore, boot *telemetry.Logger) {
...
// moduleSelection decides which modules to wire in (platform#38). It is read from
// the environment, the same bridge the DSN and log level take until a config
// pipeline owns it, and unset means every module — so an unconfigured deployment
// gets the full set and works out of the box. A selection is validated against
// what the binary carries before anything is constructed, so a typo names the
// mistake rather than silently dropping the module it misspelled.
func moduleSelection(httpClient *http.Client) (app.Selection, error) {
	selection := app.SelectAll()
	if spec, ok := os.LookupEnv(moduleSelectionEnv); ok {
		selection = app.ParseSelection(spec)
	}
	if err := selection.Validate(moduleIDs(httpClient)); err != nil {
		return selection, fmt.Errorf("invalid %s: %w", moduleSelectionEnv, err)
	}
	return selection, nil
}

// composeCapabilities registers the selected module capabilities the Platform can
// invoke — the composition-root stand-in for platform#4's build-time module
// selection, until the Supervisor activates a Generation with a selection.
//
// Verify fails boot when a capability declares a provider role it does not
// implement (sdk#2): a role named but unbacked would otherwise surface as a nil
// provider at invocation, not at composition. RequireComposedRoleClasses then
// fails boot when a required role class is unfilled over the composed set — core
// and extension together (platform#38 re-expressing platform#23) — because a Mosaic
// that cannot identify or find content is inert rather than degraded. Its
// required roles come from the role-class table (app.RoleClasses) rather than a
// hand-kept list, and it binds the serving composition and not the app.Service
// constructor, so tests that build a service directly are unaffected.
func composeCapabilities(selection app.Selection, httpClient *http.Client, boot *telemetry.Logger) (*app.CapabilityRegistry, error) {
	capRegistry := app.NewCapabilityRegistry()
	registerCapabilities(capRegistry, selection, httpClient, boot)
	if err := capRegistry.Verify(); err != nil {
		return nil, fmt.Errorf("capability registry invalid: %w", err)
	}
	if err := capRegistry.RequireComposedRoleClasses(); err != nil {
		return nil, fmt.Errorf("required capability missing: %w", err)
	}
...
// reportExtensionRepositoryTrust establishes the default extension-module trust:
// the official repository, trusted by default with the key compiled into this
// binary (platform#40, platform#49). Building it here validates the embedded key at
// boot — a corrupt or empty key fails now rather than at a user's first install —
// and records which repository the deployment trusts.
func reportExtensionRepositoryTrust(boot *telemetry.Logger) error {
...
// reportEgressContainment records the layer-3 egress-containment posture
// (platform#39, platform#50). The Platform cannot enforce OS-level egress denial from
// inside its own non-root process, so it reports which posture this deployment is
// in rather than claim the guarantee uniformly: enforced only where the OS can
// deny a module direct egress and the deployment attests it did. Logged at boot so
// the claim is visible and honest, and carried in the diagnostic support bundle.
func reportEgressContainment(boot *telemetry.Logger) {
...
// newExtensionInstaller opens the directory installed extension modules live in.
func newExtensionInstaller() (*extension.Installer, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("extension installer: %w", err)
	}
	return installer, nil
}

// newInstanceIdentity opens this install's identity (platform#54), before the
// Service because the Service reads it on the one call made before a client has
// anything — the pre-session bootstrap, which puts the server's name on its door.
func newInstanceIdentity() *instance.File {
	instanceFile := os.Getenv(instanceFileEnv)
	if instanceFile == "" {
		instanceFile = defaultInstanceFile
	}
	return instance.NewFile(instanceFile)
}

// applyConfigWaitingForRestart applies a configuration change that needed a
// restart: this is the restart it was waiting for, so it runs before anything
// reads config (supervisor#1's escalation, recorded as Pending when the user
// asked). A Generation-class change is left alone — a restart was never going to
// carry it, and the Supervisor is what can.
//
// It has its own context, and a failure is not fatal: a Platform that refuses to
// boot because a queued configuration change could not be applied is one an
// operator cannot reach to withdraw the change.
func applyConfigWaitingForRestart(svc *app.Service, boot *telemetry.Logger) {
...
// adoptInstalledExtensions re-adopts the set a user installed (platform#51); a
// fresh install has adopted nothing. One that fails to adopt is a degraded
// capability the Manager logs and skips, never a boot failure: extensions fill
// no required role class (that is core's guarantee, platform#23/0072), so one
// absent is the ordinary degraded state, not the inert Platform that
// RequireComposedRoleClasses refuses.
func adoptInstalledExtensions(extManager *extensions.Manager) error {
...
// newArtworkSigner keys the artwork proxy (platform#20), which re-serves remote
// poster/backdrop images from the Platform's origin so a client gets same-origin
// (CORS-clean) artwork. The key is process-scoped: emitted screens are re-fetched,
// so a signature need not outlive the process.
func newArtworkSigner() (*artwork.Signer, error) {
	artworkKey := make([]byte, 32)
	if _, err := rand.Read(artworkKey); err != nil {
		return nil, fmt.Errorf("generate artwork key failed: %w", err)
	}
	return artwork.NewSigner(artworkKey), nil
}

// newPlaybackSealer keys the playback origin (platform#25), which relays a resolved
// stream from the Platform's own origin so a client never holds the upstream URL
// — which for a debrid link carries a credential. Its key is process-scoped for
// the same reason the artwork signer's is, and more so: a ticket is minted when
// playback starts and a restart re-resolves rather than replaying a stale
// upstream.
func newPlaybackSealer() (*playback.Sealer, error) {
	playbackKey := make([]byte, 32)
	if _, err := rand.Read(playbackKey); err != nil {
		return nil, fmt.Errorf("generate playback key failed: %w", err)
	}
	playbackSealer, err := playback.NewSealer(playbackKey)
	if err != nil {
		return nil, fmt.Errorf("build playback sealer failed: %w", err)
	}
	return playbackSealer, nil
}

// newPlaybackTooling finds ffmpeg and ffprobe, for stream-copy remux
// (platform#27). MSE takes only fMP4/WebM, so a Matroska release is unplayable in
// a browser whatever codec is inside; rewriting the container costs almost
// nothing since the streams are copied. Both are optional — without them the
// Platform still boots and direct-plays, saying so rather than failing obscurely.
func newPlaybackTooling(boot *telemetry.Logger) (*playback.Remuxer, *playback.Prober) {
	...
	return playbackRemuxer, playbackProber
}

// reconcileOwnerRole keeps the install owner's authority current (platform#44).
//
// Every other role is a snapshot and stays one — an account created before an
// action existed does not silently gain it — and the owner's is the one that must
// not be, because it is the root of every other grant. Run unconditionally, before
// and independently of the environment-variable bootstrap: a server claimed
// through the setup wizard (platform#54) has an owner that bootstrap never sees, and
// leaving it out meant an upgrade could add an action nobody on the install could
// ever hold.
func reconcileOwnerRole(set *postgres.ContractSet, boot *telemetry.Logger) error {
...
// bootstrapAdmin optionally seeds the first administrator. There is no in-band
// way to grant the very first authority — every command that could is itself
// policy-gated — so this bridges that gap for initial setup, gated on both env
// vars being present and idempotent once the user exists. The password is read
// once and never logged, and the username is classified rather than printed:
// Identifier rather than Sensitive, so two records about the same administrator
// can still be tied together without the log holding who they are.
func bootstrapAdmin(set *postgres.ContractSet, boot *telemetry.Logger) error {
	adminUser := os.Getenv(bootstrapAdminUserEnv)
	if adminUser == "" {
		return nil
	}
...
// startBackgroundJobs starts the Platform's own background work (platform#13's
// system principal, platform#36's retention sweep). The runner claims from
// PostgreSQL with FOR UPDATE SKIP LOCKED and the scheduler enqueues each recurring
// kind's current occurrence under an idempotency key, so starting it at boot is
// also how a restart resumes: there is no state held here to recover.
//
// The library pass's schedule is configuration (platform#60) and its field is
// Restart-class, so it is read once, here, where there is a boot context to read
// it with. The budget is not read here: it is Hot, and the run reads it for itself
// every time.
func startBackgroundJobs(serveCtx context.Context, svc *app.Service, set *postgres.ContractSet, bootID string, boot *telemetry.Logger) compositionjobs.Runtime {
...
// flushQueryableSinks closes the queryable sinks last, after the shutdown health
// snapshot, so the records describing shutdown are themselves stored rather than
// lost to the thing that stores them closing first. The file sink has had them all
// along; this is about the queryable copy agreeing.
//
// Losses are reported before the flush, since reporting one is itself a record. A
// sink that silently loses records looks exactly like a quiet system, and the
// difference matters most during whatever caused it.
func flushQueryableSinks(bufferedSink *telemetry.BufferedSink[telemetry.Record], spanSink *telemetry.BufferedSink[telemetry.SpanRecord], boot *telemetry.Logger) {
