...
**Six repositories, all siblings on disk** (`../mosaic-platform`, `../mosaic-architecture`, `../mosaic-sdk`, `../mosaic-module-stremio`, `../mosaic-shell`, `../mosaic-sdui`):
...
- **`mosaic-sdui`** — the **published SDUI contract** the Platform, Modules and Shell share: JSON-Schema schema (`UINode`/`Action`/`ComponentDefinition`), a **Go producer binding** (`github.com/mosaic-media/mosaic-sdui/sdui` — the emit-side builds screens against it), the standard definition library as data, and DTCG tokens. Apache-2.0, like the SDK (ADR 0025). Wire it with a `replace => ../mosaic-sdui` until tagged. When you build the Platform's SDUI emit-side, build it against this. Commit and push it separately.
