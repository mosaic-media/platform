...
**The index generator and the citation lint are vendored into `scripts/`, and
this repository's gate runs them** — `verify.yml` and the compose file both do,
so a stale index or an unresolvable citation refuses a push here rather than
waiting for somebody to run a sibling's tooling. **Do not edit either script
here.** Their source is `architecture/scripts/`; a copy that drifts is a gate
quietly enforcing a rule that has moved, which has already happened once. Change
them there and re-vendor with `architecture/scripts/vendored_scripts.py`.
